...
class MemoryRepository:
    ...
    def create_memory(memory_create: MemoryCreate) -> Memory:
        ...
        with get_db_connection() as conn:
            ...
            embedding_bytes = np.array(
                memory_create.embedding, dtype=np.float32
            ).tobytes()

            cursor = conn.execute(
                """
                INSERT INTO memory_text
                (session_id, prompt_text, answer_text,
                timestamp, embedding, summary_text)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def get_memories_by_session(session_id: str, limit: int = 10) -> list[Memory]:
        ...
        with get_db_connection() as conn:
            cursor = conn.execute(
                """
                SELECT id, session_id, prompt_text, answer_text,
                timestamp, embedding, summary_text
                FROM memory_text
                WHERE session_id = ?
    ...
    def query_similar_by_embedding(
        ...
        with get_db_connection() as conn:
            ...
            for row in rows:
                ...
                norm_product = np.linalg.norm(target_vec) * np.linalg.norm(
                    stored_embedding
                )
    ...
    def delete_collection(collection_name: str) -> bool:
        ...
        with get_db_connection() as conn:
            conn.execute(
                "DELETE FROM memory_text WHERE session_id = ?", (collection_name,)
            )
