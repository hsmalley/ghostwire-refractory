...
def compute_general_ghostwire_score(
    ...
    normalized_memory = 1 / (
        1 + memory_usage
    )  # Approaches 1 as memory usage approaches 0
...
def compute_rag_ghostwire_score(
    ...
    normalized_hallucination = (
        1 - hallucination
    )  # Invert since lower hallucination is better
...
def format_benchmark_results_with_scores(
    results: dict[str, Any], score_function_name: str = "general"
) -> str:
    """
    ...
    try:
        if score_function_name == "general":
            ...
            latency = results.get(
                "embedding_latency",
                results.get("search_latency", results.get("storage_latency", 1.0)),
            )
            ...
            score = score_function(
                quality=quality, hallucination=hallucination, latency=latency
            )
        elif score_function_name == "comprehensive":
            latency = results.get(
                "embedding_latency", results.get("search_latency", 1.0)
            )
        ...
        result_str = "📊 BENCHMARK RESULTS WITH GHOSTWIRE SCORES:\n"
        result_str += "-" * 60 + "\n"
        ...
        result_str = (
            f"📊 BENCHMARK RESULTS (GHOSTWIRE score computation failed: {e}):\n"
        )
        result_str += "-" * 60 + "\n"
