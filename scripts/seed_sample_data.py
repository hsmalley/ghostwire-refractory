...
def seed_sample_data(db_path: str = None, embed_dim: int = None, force: bool = False):
    ...
    try:
        ...
        if not force:
            cursor.execute("SELECT COUNT(*) FROM memory_text WHERE session_id LIKE 'session_%'")
        ...
        print(f"✅ Successfully inserted {total_samples} sample memory entries across {len(sample_sessions)} sessions")
