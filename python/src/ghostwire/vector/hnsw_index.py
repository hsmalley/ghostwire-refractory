...
class HNSWIndexManager:
        ...
        with self._lock:
            ...
            if os.path.exists(self._hnsw_path):
                try:
                    ...
                    print(f"[HNSW] Loaded persistent vector index from {self._hnsw_path}.")
                    return
    ...
    def knn_query(self, query_vectors: np.ndarray, k: int = 5) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def delete_from_index(self, ids: List[int]) -> bool:
        ...
        print("[HNSW] Delete operation not supported by hnswlib. Consider rebuilding index.")
        return False
