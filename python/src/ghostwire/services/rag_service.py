...
class RAGService:
    ...
    async def retrieve_context(self, session_id: str, query: str, top_k: int = 5) -> List[str]:
        ...
        try:
            ...
            self.logger.info(f"Retrieved {len(contexts)} contexts for session {session_id}")
            return contexts
    ...
    async def generate_response(
        ...
        target_url = f"{settings.REMOTE_OLLAMA_URL}/api/generate" if use_remote else f"{settings.LOCAL_OLLAMA_URL}/api/generate"
