...
class MemoryService:
    ...
    def create_memory(self, memory_create: MemoryCreate) -> Memory:
        ...
        if len(memory_create.embedding) != settings.EMBED_DIM:
            raise EmbeddingDimMismatchError(settings.EMBED_DIM, len(memory_create.embedding))
        ...
        try:
            ...
            self.hnsw_manager.add_items(
                vec.reshape(1, -1),
                np.array([memory.id], dtype=np.int64)
            )
            ...
            try:
                ...
                if ids:
                    ...
                    all_memories = self.repository.get_memories_by_session(
                        query.session_id, limit=100  # reasonable limit
                    )
                    ...
                    self.logger.info(f"[HNSW] Retrieved {len(result)} neighbors from HNSW index.")
                    return result
            except Exception as e:
                self.logger.error(f"[HNSW] Query failed ({e}); falling back to cosine similarity.")
