...
from src.ghostwire.services.memory_service import MemoryService
from src.ghostwire.models.memory import MemoryCreate
...
class TestMemoryService:
    ...
    @patch('src.ghostwire.database.repositories.MemoryRepository.query_similar_by_embedding')
    @patch('src.ghostwire.vector.hnsw_index.get_hnsw_manager')
    def test_query_similar_memories(self, mock_get_hnsw_manager, mock_query_db):
        ...
        mock_hnsw_manager.get_current_count = MagicMock(return_value=0)  # No HNSW results
        mock_get_hnsw_manager.return_value = mock_hnsw_manager
