...
def validate_text_content(text: str, max_length: int = 10000) -> bool:
    ...
    if len(text) > max_length:
        raise ValidationError(f"Text content too long. Maximum {max_length} characters allowed")
    ...
    for pattern in injection_patterns:
        if re.search(pattern, text, re.IGNORECASE):
            raise ValidationError(f"Text content contains potentially unsafe patterns: {pattern}")
...
def validate_embedding(embedding: Any, expected_dim: int) -> bool:
    ...
    if len(embedding) != expected_dim:
        raise ValidationError(f"Embedding dimension {len(embedding)} does not match expected {expected_dim}")
    
    # Check that all elements are numbers
    for i, val in enumerate(embedding):
        if not isinstance(val, (int, float)):
            raise ValidationError(f"Embedding value at index {i} is not a number: {type(val)}")
