...
class EmbeddingService:
    ...
    async def _get_embedding_from_api(self, text: str, model: str) -> List[float]:
        ...
        try:
            ...
            if not embedding and "embeddings" in data and isinstance(data["embeddings"], list):
                embedding = data["embeddings"][0]
            
            if embedding:
                return embedding
        except Exception as e:
            self.logger.warning(f"Failed to get embedding from /api/embeddings for model {model}: {e}")
        ...
        try:
            ...
            self.logger.error(f"Failed to get embedding from /api/embed for model {model}: {e}")
    ...
    async def create_embedding(self, request: EmbeddingRequest) -> EmbeddingResponse:
        ...
        for i, text_input in enumerate(inputs):
            ...
            if self._cached_embed_model:
                embedding_vector = await self._get_embedding_from_api(text_input, self._cached_embed_model)
            
            # If still no embedding, try all available models
            if not embedding_vector:
                for model_name in settings.EMBED_MODELS:
                    embedding_vector = await self._get_embedding_from_api(text_input, model_name)
                    if embedding_vector:
            ...
            if len(embedding_vector) != settings.EMBED_DIM:
                ...
                if len(embedding_vector) < settings.EMBED_DIM:
                    embedding_vector.extend([1e-8] * (settings.EMBED_DIM - len(embedding_vector)))
                else:
        ...
        self.logger.info(f"Successfully generated {len(inputs)} embeddings via {model or self._cached_embed_model}")
        return response
