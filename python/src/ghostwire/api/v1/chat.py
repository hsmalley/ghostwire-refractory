...
from ...utils.security import validate_session_id, validate_text_content, validate_embedding
from ...utils.error_handling import handle_exception, EmbeddingDimMismatchError
    ...
    try:
        ...
        if not embedding:
            ...
            if not embedding:
                raise HTTPException(
                    status_code=500, 
                    detail="Failed to auto-generate embedding"
                )
...
async def add_memory(request: ChatEmbeddingRequest):
    ...
    try:
        ...
        # Validate text content
        if text:
            validate_text_content(text, max_length=10000)  # Larger limit for memory storage
