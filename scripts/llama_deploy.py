...
class LlamaDeployer:
    ...
    def deploy_llamafile(
        ...
        """
        logger.info(
            f"🚀 Deploying llamafile '{llamafile_path}' as service '{service_name}'"
        )
        ...
        for host in self.hosts:
            try:
                if self._deploy_to_host(
                    host, llamafile_path, service_name, destination_path, force
                ):
                    success_count += 1
        ...
        logger.info(
            f"🏁 Deployment complete: {success_count}/{len(self.hosts)} hosts successful"
        )
        ...
        try:
            ...
            if not force:
                ...
                if result.return_code == 0:
                    logger.warning(
                        f"⚠️ Service '{service_name}' already active on {host}. Use --force to redeploy."
                    )
                    return True
            ...
            # Upload llamafile
            remote_llamafile_path = os.path.join(
                destination_path, os.path.basename(llamafile_path)
            )
            ...
            unit_content = self._generate_systemd_unit(
                service_name, remote_llamafile_path
            )
...
Examples:
  ...
  %(prog)s --hosts server1 --check-health
    ...
    parser.add_argument("--llamafile", type=str, help="Path to the llamafile to deploy")
    ...
    parser.add_argument(
        "--user", type=str, help="SSH username (defaults to current user)"
    )
    ...
    if args.check_health:
        ...
        health_status = deployer.check_health(
            service_name=args.service_name, port=args.health_port
        )
        ...
        if success:
            ...
            print(
                f"   {parser.prog} --hosts {','.join(hosts)} --check-health --service-name {args.service_name}"
            )
            sys.exit(0)
        else:
