...
class SummarizationBenchmark:
    ...
    async def test_summarization(
        ...
        return avg_latency, latencies, avg_memory_delta, quality, hallucination_rate, length_penalty
    ...
    async def run_summarization_benchmark(self):
        ...
        for i, text in enumerate(test_texts):
            ...
            avg_lat, latencies, avg_memory, quality, hallucination_rate, length_penalty = await self.test_summarization(text, iterations=3)
        ...
        overall_avg_latency = sum(latency_results) / len(latency_results) if latency_results else float("inf")
        overall_avg_memory = sum(memory_results) / len(memory_results) if memory_results else 0.0
        overall_avg_quality = sum(quality_results) / len(quality_results) if quality_results else 0.0
        overall_avg_hallucination = sum(hallucination_results) / len(hallucination_results) if hallucination_results else 0.0
        overall_avg_length_penalty = sum(length_penalty_results) / len(length_penalty_results) if length_penalty_results else 0.0
        ...
        general_ghostwire_score = compute_general_ghostwire_score(
            latency=overall_avg_latency,
            memory_usage=overall_avg_memory
        )
...
async def main():
    ...
    print(f"  Overall Average Hallucination Rate: {results['average_hallucination_rate']:.4f}")
    print(f"  Overall Average Length Penalty: {results['average_length_penalty']:.4f}")
    print(f"  Summarization GHOSTWIRE Score: {results['summarization_ghostwire_score']:.4f}")
    print(f"  General Performance GHOSTWIRE Score: {results['general_performance_ghostwire_score']:.4f}")

    # Also print formatted results with GHOSTWIRE scoring
    print("\n" + format_benchmark_results_with_scores(results, score_function_name="summarization"))
