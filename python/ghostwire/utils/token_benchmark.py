...
class TokenBenchmarkSuite:
    ...
    async def benchmark_caching_layer(self):
        ...
        for query in test_queries:
            ...
            response_tokens = (
                estimate_token_count("This is a typical response to the query.") * 3
            )  # 3x for elaboration
        ...
        result = TokenBenchmarkResult(
            ...
            savings_percentage=((baseline_tokens - optimized_tokens) / baseline_tokens)
            * 100,
        ...
        logger.info(
            f"Caching layer benchmark completed: {result.savings_percentage:.1f}% token savings"
        )
    ...
    async def benchmark_context_window_optimization(self):
        ...
        medium_context = (
            "This is a medium-length context with some detailed information. " * 10
        )
        long_context = (
            "This is a very long context with extensive detailed information. " * 50
        )
        ...
        test_contexts = [
            ...
            medium_context,
            long_context,
        ] * 3  # Multiple contexts to optimize
            ...
            * 100,
            ...
            details={
                ...
                "average_context_reduction": (
                    (len(test_contexts) - len(optimized_contexts)) / len(test_contexts)
                )
                * 100,
            },
        )
        ...
        self.results.append(result)
        logger.info(
            f"Context window optimization benchmark completed: {result.savings_percentage:.1f}% token savings"
        )
    ...
    async def benchmark_summarization_optimization(self):
        ...
        long_text = (
            "This is a very long text that contains extensive information. " * 100
        )
        ...
        optimized_tokens = estimate_token_count(
            "A" * target_length
        )  # Approximate compressed text
        ...
        self.results.append(result)
        logger.info(
            f"Summarization optimization benchmark completed: {result.savings_percentage:.1f}% token savings"
        )
    ...
    async def benchmark_response_caching(self):
        ...
        baseline_tokens = (
            query_tokens + estimate_token_count("Typical weather response")
        ) * 10
        ...
        result = TokenBenchmarkResult(
            ...
            * 100,
            ...
            details={
                ...
                "tokens_saved_per_cache_hit": estimate_token_count(
                    "Typical weather response"
                ),
            },
        )
        ...
        self.results.append(result)
        logger.info(
            f"Response caching benchmark completed: {result.savings_percentage:.1f}% token savings"
        )
    ...
    async def benchmark_end_to_end_optimization(self):
        ...
        for query in test_queries:
            ...
            context_tokens = estimate_token_count(
                "Extensive context information retrieved from memory. " * 20
            )
            baseline_tokens += context_tokens

            # Response generation
            response_tokens = estimate_token_count(
                "Detailed response with comprehensive information. " * 15
            )
        ...
        for i, query in enumerate(test_queries):
            ...
            if i == 0:
                ...
                context_tokens = sum(
                    estimate_token_count(ctx) for ctx in reduced_contexts
                )
        ...
        self.results.append(result)
        logger.info(
            f"End-to-end optimization benchmark completed: {result.savings_percentage:.1f}% token savings"
        )
    ...
    def generate_report(self) -> str:
        ...
        overall_savings = (
            ((total_baseline - total_optimized) / total_baseline) * 100
            if total_baseline > 0
            else 0
        )
